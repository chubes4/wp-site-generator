...
- Codebox runtime profiles set `runtime_task_ability`, `runtime_bundle_ability`, and `runtime_workflow_ability` to `agents/run-runtime-package` until WP Codebox exposes a canonical package-execution wrapper for that Agents API entry point.
