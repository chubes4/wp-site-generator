...
- WP Codebox/Homeboy still need generic GitHub issue creation and review-comment callback primitives, plus generic evidence/output projection inputs for agent-task runs. Exact dependency: WP Codebox `origin/main` still normalizes agent-task workloads from `agent_bundle.engine_data_outputs` and `agent_bundle.tool_recorders` in `packages/cli/src/commands/agent-task-run.ts`, and Homeboy Extensions `origin/main` still forwards `engine_data_outputs` and `tool_recorders` through `agent-runtimes/wp-codebox/lib/codebox-agent-task-executor.js`, `datamachine-agent-ci/lib/datamachine-agent-ci-plan.js`, and `.github/workflows/datamachine-agent-ci.yml`. Owner/gap: the merged WP Codebox provider runtime invocation contract covers runner workspace command/publication, tool-call transcript recording, artifact handoff, and runtime evidence result schemas, but it does not yet cover issue/comment callbacks or generic agent-task projection config replacing those legacy keys.
