...
## Review Rubric
1. **Correct owner repo**: the PR belongs in the repository that owns the root behavior. SSI import/source-selection/asset-map behavior belongs to `static-site-importer`; HTML-to-block conversion, format bridge behavior, block serialization/report projection, and artifact compiler behavior belong to `Automattic/blocks-engine` under `php-transformer`; generated-source policy or visual brittleness belongs to `wp-site-generator` only when the packet proves the source site is the root cause.
