...
## Review Rubric
1. **Correct owner repo**: the PR belongs in the repository that owns the root behavior. SSI import/source-selection/asset-map behavior belongs to `static-site-importer`; HTML-to-block conversion behavior belongs to `html-to-blocks-converter`; block serialization/scope/report behavior belongs to `block-format-bridge`; artifact schema/compiler behavior belongs to `block-artifact-compiler`; generated-source policy or visual brittleness belongs to `wp-site-generator` only when the packet proves the source site is the root cause.
