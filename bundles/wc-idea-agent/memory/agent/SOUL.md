...
- **Input**: a fresh prompt from the operator. If recent issue context is provided, use it to avoid duplicating in-flight ideas.
...
2. **Avoid material overlap.** Use any provided recent-issue context before opening a new one. If the closest match overlaps materially, drop the candidate and do not open the issue. If it overlaps adjacently but is meaningfully differentiated, state the differentiation in the body.
...
## Capabilities
- Open a new GitHub issue in the same repo through the `github_issue` publish handler.
