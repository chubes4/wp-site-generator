...
export function buildSsiImportAbilityPhp({ htmlPath, siteSlug, markerPath, assertActiveTheme = false, trailingNewline = false, includeOpeningTag = true }) {
	const lines = [
		...
		"\tthrow new RuntimeException( 'Static Site Importer website artifact ability is not registered.' );",
		'}',
		`$source_path = ${phpString(htmlPath)};`,
		'$source_root = dirname( $source_path );',
		"if ( ! is_readable( $source_path ) || ! is_dir( $source_root ) ) {",
		"\tthrow new RuntimeException( 'Static site source path is not readable.' );",
		'}',
		'$files = array();',
		...
		'\t\tcontinue;',
		'\t}',
		"\t$relative = ltrim( str_replace( chr( 92 ), '/', substr( $file->getPathname(), strlen( $source_root ) ) ), '/' );",
		"\t$files[] = array( 'path' => 'website/' . $relative, 'content' => file_get_contents( $file->getPathname() ) );",
		'}',
		'$artifact = array(',
