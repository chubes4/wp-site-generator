#!/usr/bin/env bash
#
# Production CI path: import and run the wc-idea-agent in WordPress
...
if [ ! -d "$BUNDLE_SOURCE" ]; then
    echo "ERROR: wc-idea-agent bundle not found" >&2
    exit 1
fi
...
RESULTS_TMPFILE=$(mktemp "${TMPDIR:-/tmp}/wc-site-generator-idea-agent.XXXXXX")
CONFIG_TMPFILE=$(mktemp "${TMPDIR:-/tmp}/wc-site-generator-idea-agent-config.XXXXXX")
...
jq -nc \
    ...
    '{
        ...
        workload_id: "dm-wc-idea-agent",
        workload_label: "Run imported wc-idea-agent and publish a concept issue",
        ...
        github_profile_id: "wc-idea-agent-ci",
...
scenario='.scenarios[] | select(.id == "dm-wc-idea-agent")'
...
if [ "$import_resolved" = "true" ] \
    ...
    && [ -n "$issue_url" ]; then
    echo "wc-idea-agent PASSED - opened $issue_url"
    exit 0
fi

echo "wc-idea-agent FAILED - see envelope above"
exit 1
