...
assert.match(workflow, /design-store-issue:[\s\S]*flow_step_patches: '\[\{"step_type":"fetch","merge":\{"handler_configs":\{"github":\{"issue_number":\$\{\{ needs\.collect-store-issue\.outputs\.issue_number \}\},"max_items":1/, 'store design fetch is scoped to the store issue');
assert.match(workflow, /design-website-issue:[\s\S]*flow_step_patches: '\[\{"step_type":"fetch","merge":\{"handler_configs":\{"github":\{"issue_number":\$\{\{ needs\.collect-website-issue\.outputs\.issue_number \}\},"max_items":1/, 'website design fetch is scoped to the website issue');
