...
try {
	...
	const runFromSpecCommand = evidence.commands.find((item) => item.command.includes('agent-task controller from-spec'));
	assert.ok(runFromSpecCommand, 'evidence records Homeboy from-spec command');
	assert.match(runFromSpecCommand.command, /--runner homeboy-lab .*agent-task controller from-spec/, 'validator routes Homeboy through the requested runner before the from-spec command');
	assert.ok(runFromSpecCommand.command.includes('--output'), 'validator asks Homeboy to write structured from-spec output');
	assert.match(runFromSpecCommand.command, /agent-task controller from-spec .* --output /, 'from-spec output remains a command argument, not a wrapper/global option');
	...
	assert.ok(evidence.commands.every((item) => !item.command.includes('agent-task controller init')), 'headless path does not manually initialize controllers');
