...
assert.equal(controller.workflows.find((workflow) => workflow.workflow_id === 'static-validation').runtime_execution.kind, 'command', 'static validation runs as a deterministic controller action');
assert.deepEqual(controller.workflows.find((workflow) => workflow.workflow_id === 'static-validation').artifacts, ['static_validation_run', 'import_validation_result', 'visual_parity_artifact', 'finding_packet_set'], 'static validation artifacts declare emitted outputs only');
assert.equal(controller.workflows.find((workflow) => workflow.workflow_id === 'static-publication-gate').runtime_execution.kind, 'command', 'static publication gate runs as a deterministic controller action');
...
const missingRuntimeAccessResult = spawnSync(process.execPath, ['.github/scripts/build-static-preview-blueprint.mjs', '--site', 'issue-123-native-loop', '--source-repo', 'chubes4/wp-site-generator', '--source-head-sha', 'b'.repeat(40), '--output', path.join(tempDir, 'missing-runtime-access.json')], {
	cwd: repoRoot,
	encoding: 'utf8',
});
assert.notEqual(missingRuntimeAccessResult.status, 0, 'preview generation fails closed without runtime access evidence');
assert.match(missingRuntimeAccessResult.stderr, /runtime access evidence refs|HOMEBOY_RUNTIME_PREVIEW_URL/, 'preview failure explains required runtime access evidence');
