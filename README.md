...
The Lab-native loop is represented by `.github/homeboy/controllers/static-site-generation-loop.controller.json`: concept packets, design packets, static-site candidates, import validation, PR publication, static validation, visual parity, finding packets, upstream iterator PRs, revalidation attempts, and reviewer gate outcomes are all controller lineage entities. Static sites arrive as **pull requests**: raw HTML/CSS/assets that are imported through Static Site Importer in Homeboy CI or Lab, with metrics, visual parity artifacts, finding packets, and a Playground preview posted back as review evidence.
