...
Ideas can still live as **GitHub issues** on the compatibility path. The Lab-native loop is represented by `.github/homeboy/controllers/static-site-generation-loop.controller.json`: concept packets, design packets, static-site candidates, import validation, PR publication, static validation, visual parity, finding packets, upstream iterator PRs, and reviewer gate outcomes are all controller lineage entities. Static sites arrive as **pull requests**: raw HTML/CSS/assets that are imported through Static Site Importer in Homeboy CI or Lab, with metrics, visual parity artifacts, finding packets, and a Playground preview posted back as review evidence.
...
This repo is the only place all of them meet. The Homeboy controller spec is the shared contract for Lab execution; GitHub Actions workflows are trigger/reporting compatibility surfaces that should point back to that contract instead of owning native control flow.
...
Static-site PRs trigger a Homeboy CI workflow today. In the Lab-native loop, the same stages are driven by `.github/homeboy/controllers/static-site-generation-loop.controller.json` with `codebox` backend execution and in-sandbox Codex provider assumptions.
